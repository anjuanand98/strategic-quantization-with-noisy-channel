...
for M=Mval % loop over M
    ...
    save(strcat('x1.5_noiseless_gaussian_fmincon1_data','M',num2str(M),'.mat'),'x_opt','y_opt','e_opt','d_opt','M','xmall','endist','dedist','exitm','xinitm','xrnall','xrninitall','enall');
end
save(strcat('x1.5_noiseless_gaussian_fmincon1_data.mat'),'Mval','xmall','endist','dedist','exitm','xinitm','xrnall','xrninitall','enall');
...
f22=0;
for m=1:M
    f22=f22+ integral(@(xv)(1.5*xv-ym(m))^2*f1(xv),x(m),x(m+1),'ArrayValued',true);
end