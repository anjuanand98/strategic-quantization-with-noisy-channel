...
%%
% parameters
rn=10; % # of random initializations
Mval=[8]; % # of levels of quantization
rhoval=[0.99]; % correlation
p_b=0.2; % bit error rate
...
save(strcat('nonstr_fmincon1_data_nt',num2str(nt),'noisy',num2str(p_b),'rho',num2str(rho),'M',num2str(M),'.mat'),'rho','p_b','x_opt','y_opt','e_opt','d_opt','thval','M','xmall','endist','dedist','exitm','xinitm','xrnall','xrninitall','enall');
end
...
save(strcat('nonstr_fmincon1_data_nt',num2str(nt),'noisy',num2str(p_b),'rho.mat'),'thval','rhoval','Mval','p_b','xmall','endist','dedist','exitm','xinitm','xrnall','xrninitall','enall');
...
for i=1:M
    for t=1:length(thval)
            f22=f22+c_2*integral(@(xv)(xv-ym(i))^2*f1(xv,t),x((t-1)*(M+1)+i),x((t-1)*(M+1)+i+1),'ArrayValued',true);
        for j=1:M
            f22=f22+c_1*integral(@(xv)(xv-ym(j))^2*f1(xv,t),x((t-1)*(M+1)+i),x((t-1)*(M+1)+i+1),'ArrayValued',true);
        end
    end
end
